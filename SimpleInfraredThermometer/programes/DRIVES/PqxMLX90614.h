#ifndef _PQXMLX90614_H_
#define _PQXMLX90614_H_
#include <intrins.h>
#include <rtx51tny.h>
#include "STC15F2K60S2.h"
#include "PqxDelay.h"
//#include "LCD12864.h"
#define SCL P26
#define SDA P27
#define true  1
#define false 0
#define		_SDA_OUTPUT() 	P2M1 |= 0x80;P2M0 |=0x80
#define		_SDA_INPUT()  	P2M1 |= 0x80;P2M0 &=~0x80
#define		_SCL_OUTPUT() 	P2M1 |= 0x40;P2M0 |=0x40

//MLX90614命令模式Command Mode
#define RamAccess            0x00	//对RAM操作
#define EepromAccess         0x20	//对EEPROM操作
#define CommandMode          0x60	//进入命令模式
#define ExitCommandMode      0x61	//退出命令模式
#define ReadFlag	           0xf0	//读标志
#define EnterSleep           0xff	//进入睡眠模式，最后一步SCL=0,退出睡眠SCL=1、SDA=0 >33ms;
//MLX90614的RAM地址，都是只读的
#define AbmientTempAddr      0x03	//周围环境温度
#define IR1Addr              0x04	//
#define IR2Addr              0x05	//
#define LineAbmientTempAddr  0x06	//环境温度
/*0x0000 0x4047    16500    0.01/单元    -40 125*/
#define LineObj1TempAddr     0x07 //目标温度，红外温度
/*0x27ad-0x7fff 0x3559    22610    0.02/单元    -70.01-382.19 0.01 452.2*/
#define LineObj2TempAddr     0x08 //
//MLX90614的EEPROM地址
#define TObjMaxAddr          0x00 //测量范围上限
#define TObjMinAddr          0x01 //测量范围下限
#define PWMCtrLAddr          0x02 //PWM设定
#define TaRangeAddr          0x03 //环境温度设定
#define KeAddr               0x04 //频率修正系数，发射率修正系数
#define ConfigAddr           0x05 //配置寄存器
#define SMbusAddr            0x0e //器件地址设定
#define Reserver1Addr        0x0f //保留地址1
#define Reserver2Addr        0x19 //保留地址2
#define ID1Addr              0x1c //ID地址1
#define ID2Addr              0x1d //ID地址2
#define ID3Addr              0x1e //ID地址3
#define ID4Addr              0x1f //ID地址4

static unsigned char code CRC8TAB[256]={
		
};

static void Pwm2Smbus(void);
static void start_bit(void);
static void stop_bit(void);
static void SmbusAck(void);
static void SmbusNack(void);
static bit send_byte(unsigned char dat);
static unsigned char receive_byte(void);
void initMLX90614(void);
short PqxMemRead(unsigned char slaveAddress,unsigned char cmd);
void PqxMemWrite(unsigned char slaveAddress,unsigned char cmd,unsigned char dataH,unsigned char dataL);
void PqxEepromWrite(unsigned char slaveAddress,unsigned char cmd,unsigned char dataH,unsigned char dataL);
bit EnterSleepMode(unsigned char slaveAddress);
unsigned char CalculatePEC(unsigned char* msg,int len);
float Data2TempK(short ddata);
float Data2TempC(short ddata);
int TempC2Data(float result);

void Float2Emissivity(float e,unsigned char* dataH,unsigned char* dataL);
void ChangeEmissivity(unsigned char slaveAddress,float emissivity);

#endif